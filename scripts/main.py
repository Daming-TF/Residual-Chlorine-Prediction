...
import lib.models
from lib.dataset import ResidualChlorineDataset, data_seg1, data_seg2, data_inverse_transform
from val import val
...
def main():
    ...
    # res_cl = pandas.read_excel(r'E:\residual chlorine\test.xlsx')
    ...
    考虑到初始序列，实际上我们需要划分80个训练样本和13个测试样本
    模型验证时去除初始序列样本，实际上有效验证的训练样本有75个，有效验证的测试样本有8个
    """
    model.eval()    # 不启用 Batch Normalization 和 Dropout

    res_cl_train = data_inverse_transform(scaler, res_cl_train_norm)    # 列向量
    train_num = len(res_cl_train_norm)
    train_init_seq = res_cl_train_norm[0:window_size]
    train_pred = val(args, model, train_num, scaler, train_init_seq, res_cl_train)      # 列向量

    res_cl_test = data_inverse_transform(scaler, res_cl_test_norm)
    val_num = len(res_cl_test_norm)
    val_init_seq = res_cl_test_norm[0:window_size]
    test_pred = val(args, model, val_num, scaler, val_init_seq, res_cl_test)
